classdef DENSE3DPlugin < plugins.DENSEanalysisPlugin
    ...
    methods
        ...
        function self = DENSE3DPlugin(varargin)
            ...
            %self.hfig = figure;
            self.hfig = findall(0, 'type', 'figure');
            %
            import plugins.dense3D_plugin.*

            self.hdense = DENSE3D();

            self.Handles.datalistener = addlistener(self.hdense, 'NewData', @(s,e)self.refresh());
        ...
        function initGUI(self)
            ...
            self.Handles.hpopup = uipanel('Parent', self.hpanel, ...
                'Position', [0 0 150 200]);
            self.Handles.hpanel = uipanel('Parent', self.hpanel, ...
                'BorderType', 'none', ...
                'BackgroundColor', [0 0 0]);

            self.Handles.hpopup = uipanel('Parent', self.hpanel);
            set(self.Handles.hpopup, 'Units', 'pixels', 'Position', [0 0 200, 200]);
            ...
            self.Handles.TabListener = [];
            self.Handles.TabListener = addlistener(h.hsidebar, 'SwitchTab', @(s,e)tab(s));
