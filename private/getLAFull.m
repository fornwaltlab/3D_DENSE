function [outervert,lvert,rvert] = getLAFull(varargin)
...
%     --input parsing passed to private/PARSESLAINPUTS
    ...
    % parse inputs
    args = struct(...
        'Color',                {{'b','r',[0 0.75 0], 'y'}},...
        'ConstructionVisible',  {{'on','on','on', 'on'}},...
        'PointsPerContour',     9);
    ...
    [orect,ocont,irect,rcont,lcont] = contourrecovery(...
    ...
    pp = cscvn2(lcont','variational');
    s = linspace(0,pp.breaks(end),args.PointsPerContour);
    lvert = fnval(pp,s)';
    lvert = flipud(lvert);

    pp = cscvn2(rcont','variational');
    s = linspace(0,pp.breaks(end),args.PointsPerContour);
    rvert = fnval(pp,s)';
    rvert = flipud(rvert);
    ...
    function redraw(pos,dbl)
        ...
        set(hcontour(2),'xdata',lcont(:,1),'ydata',lcont(:,2));
        set(hcontour(3),'xdata',rcont(:,1),'ydata',rcont(:,2));
...
%% CONTOUR RECOVERY
function [orect,ocont,irect,lcont,rcont] = contourrecovery(x,y,N,flag,xlim,ylim)
...
% scalar input indicating the number of contour points to be recovered.
    ...
    % contour coordinates
    ocont = flipud(contourvertices(x(1),y(1),vmag,do,phi,N));

    xx = mean([xi(2), x(1)]);
    yy = mean([yi(2), y(1)]);


    lcont = flipud(contourvertices(xx,yy,vmag-thick,di/2.5,phi,N));

    xx = mean([xi(1), x(1)]);
    yy = mean([yi(1), y(1)]);

    rcont = flipud(contourvertices(xx,yy,vmag-thick,di/2.5,phi,N));
end
