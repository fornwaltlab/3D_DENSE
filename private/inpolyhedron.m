function [IN, distances] = inpolyhedron(varargin)
...
if ~any(ptsToConsidMask)
    IN = reshapeINfcn(IN);
    distances = reshapeINfcn(distances);
    return;
end
...
for i = find(~emptyMask)
    ...
    for ptNo = find(qPtHitsTriangles(:))'
        ...
        D = min(abs(minFacetDistanceFcn(facetToQptDists)));
        distances( outPxIndsViaUnqIndiceMask(allqPtInds(ptNo), ...
            minFacetDistanceFcn(facetToQptDists)<options.tol...
            )) = D;
    end
end
