...
%
%   The RBFInterpolator class allows the user to easily compute radial basis
%   functions for the supplied data. This class is extrenemely flexible and can
%   be used for N-D data and can simultaneously interpolate several data
%   associated with the specified points
%
    ...
    methods
        function self = RBFInterpolator(pts, vals, varargin)
            ...
            if isempty(self.Constant)
                D = max(self.Points) - min(self.Points);
                self.Constant = (prod(D) / self.NPoints)^ ...
                                (1 / size(self.Points, 2));
            end
        ...
        function B = subsref(self, S)
            ...
            %
            %   This is overloaded because we want to be able to use indices to
            %   interpolate the RBF
